...
describe("ditto dql command wiring (mocked SDK boundary)", () => {
  it("one-shot SELECT renders rows as JSON (non-TTY) and exits 0", async () => {
    ...
    await buildProgram().parseAsync(["node", "ditto", "dql", "exec", "SELECT * FROM movies", "-d", dataDir]);
  ...
  it("mutation prints OK", async () => {
    h.rows = [];
    await buildProgram().parseAsync(["node", "ditto", "dql", "exec", "INSERT INTO movies DOCUMENTS ({'_id':'1'})", "-d", dataDir]);
    expect(stdout()).toBe("OK");
  });

  it("query error exits 1 with statement excerpt", async () => {
    h.failWith = { message: "bad syntax", code: "query/invalid" };
    await buildProgram().parseAsync(["node", "ditto", "dql", "exec", "SELEC broken", "-d", dataDir]);
    expect(process.exitCode).toBe(1);
    expect(stderr()).toContain("Query error [query/invalid]: bad syntax");
  });

  it("-f missing file exits 2", async () => {
    await buildProgram().parseAsync(["node", "ditto", "dql", "exec", "-f", "/nonexistent.dql", "-d", dataDir]);
  ...
  it("bad -p pair exits 2", async () => {
    await buildProgram().parseAsync(["node", "ditto", "dql", "exec", "SELECT * FROM movies", "-p", "noequals", "-d", dataDir]);
    expect(process.exitCode).toBe(2);
  });
