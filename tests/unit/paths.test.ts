...
describe("resolveDataDir precedence", () => {
  it("flag beats env beats default", () => {
    const env = { DITTOSH_DATA_DIR: "/env/dir" } as NodeJS.ProcessEnv;
  ...
  it("trims whitespace around the flag/env value", () => {
    ...
    expect(resolveDataDir(undefined, { DITTOSH_DATA_DIR: " /tmp/y " } as NodeJS.ProcessEnv)).toBe(
      path.resolve("/tmp/y"),
    );
  });
  ...
  it("empty-string flag AND empty env fall through to default (never cwd)", () => {
    expect(resolveDataDir("", { DITTOSH_DATA_DIR: "" } as NodeJS.ProcessEnv)).toBe(
      defaultDataDir(),
    );
  ...
  it("default is an absolute path containing 'dittosh'", () => {
    const def = defaultDataDir();
    expect(path.isAbsolute(def)).toBe(true);
    expect(def.toLowerCase()).toContain("dittosh");
