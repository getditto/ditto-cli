...
describe.skipIf(!hasDevCredentials)(`integration: datasets (${NO_CREDENTIALS})`, () => {
  ...
  it("loads movies (100 docs) and runs catalog queries against it", async () => {
    ...
    const single = extractRows(await session.execute("SELECT * FROM movies WHERE _id.year = '1893'"));
    expect(single).toHaveLength(1);
    expect((single[0]!._id as Record<string, unknown>).title).toBe("Blacksmith Scene");

    const empty = extractRows(await session.execute("SELECT * FROM movies WHERE _id.year = '1800'"));
    expect(empty).toHaveLength(0);

    const starred = extractRows(await session.execute("SELECT * FROM movies WHERE _id.title LIKE '%Star%'"));
  ...
  it("loads retail (1,000 orders) with working catalog literals", async () => {
    ...
    const stores = extractRows(await session.execute("SELECT * FROM stores WHERE location.city = 'Seattle' AND deleted = false"));
    ...
    // Anchors
    const anchorOrder = extractRows(await session.execute("SELECT * FROM orders WHERE _id = 'order_20250115_0001'"));
    expect(anchorOrder).toHaveLength(1);
    const anchorCustomer = extractRows(await session.execute("SELECT * FROM customers WHERE email = 'john21@example.net'"));
    expect(anchorCustomer).toHaveLength(1);
    const anchorItems = extractRows(await session.execute("SELECT * FROM order_items WHERE order_id = 'order_20250115_0001'"));
    expect(anchorItems.length).toBeGreaterThanOrEqual(1);
  });

  it("loads retail-joins and JOINs work", async () => {
    await loadDataset(session, getDataset("retail-joins")!, { docs: 500, seed: 42, batchSize: 500 });
